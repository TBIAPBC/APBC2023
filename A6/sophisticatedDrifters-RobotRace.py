...
class AggressiveDrifter(Player):
    ...
    def move(self, status):
        ...
        best_path = my_paths.shortestPathFrom(my_pos)
        best_path = best_path[1:]
        best_path.append(gold_pos)
        ...
        for coord in best_path:
            ...
            if tile.obj is not None and tile.obj.is_player():
                best_path = self.around_player((x, y), my_paths, my_pos)
                best_path = best_path[1:]
                best_path.append(gold_pos)
                break

        distance = len(best_path)

        if distance > 6:
            numMoves = int(25 / distance)
        else:
            numMoves = distance
        ...
        return self._as_directions(my_pos, best_path[:numMoves])

    def around_player(self, coords, my_paths, my_pos):
        my_paths.map[coords] = TileStatus.Wall
...
players = [AggressiveDrifter(), EstablishedDrifter()]
